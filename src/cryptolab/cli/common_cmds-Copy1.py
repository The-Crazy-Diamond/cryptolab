...
from cryptolab.analysis import ANALYSIS_METHODS
...
def create_command(method_name: str):
    def factory(cipher_module):
        ...
        Examples:
            cryptolab (de)cipher {cipher_module.NAME} "HELLO" {cipher_module.ARGS_EXAMPLE}
            cryptolab (de)cipher {cipher_module.NAME} file.txt {cipher_module.ARGS_EXAMPLE}
        """
...
def list_analysis_methods():
    ...
    for name, module in sorted(ANALYSIS_METHODS.items()):
        desc = getattr(module, "DESCRIPTION", "")
        print(f"- {name:<15} {desc}")
